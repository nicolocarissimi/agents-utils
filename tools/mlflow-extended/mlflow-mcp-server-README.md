# MLflow MCP Server for Claude Code

Connect Claude Code to your MLflow tracking server so you can query experiments, runs, metrics, and artifacts directly from the CLI.
...
```bash
pip install fastmcp mlflow
```

## 2. Create the server script

Save the following as `~/mlflow_runs_mcp_server.py`:
...
```bash
claude mcp add mlflow-runs \
  -t stdio \
  -e MLFLOW_TRACKING_URI=http://your-mlflow-server:5000 \
  -- python3 ~/mlflow_runs_mcp_server.py
```
...
```json
{
  "mcpServers": {
    "mlflow-runs": {
      "type": "stdio",
      "command": "python3",
      "args": ["/home/your-user/mlflow_runs_mcp_server.py"],
      "env": {
...
| Tool | Description |
|---|---|
| `search_experiments` | List/filter experiments |
| `search_runs` | Query runs with filters, sorting, pagination |
| `get_run` | Full details of a single run (metrics, params, tags) |
| `get_metric_history` | Step-by-step history of a metric within a run |
| `list_artifacts` | Browse artifact tree for a run |
| `download_artifact` | Download an artifact to local disk |
...
```
> search for experiments on MLflow
> get metrics from run <run_id>
> show metric history for val_loss in run <run_id>
```

## Troubleshooting

- **Server won't start**: Check that `python3 -c "import fastmcp, mlflow"` works
- **Connection refused**: Verify `MLFLOW_TRACKING_URI` is reachable from the machine (`curl http://your-mlflow-server:5000/api/2.0/mlflow/experiments/list`)
- **Tools not showing up**: Restart Claude Code after editing `~/.claude.json`; MCP servers are loaded at startup
